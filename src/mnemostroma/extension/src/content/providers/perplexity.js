...
export default makeParser('perplexity', '1.0.0', {
  allowlist: {
  ...
  matchRequest(meta) {
    return meta.method === 'POST' && (meta.url.includes('/api') || meta.url.includes('/chat/completions'));
  },
  ...
  finalize(context) {
    ...
    const confidence = assistantText ? 0.88 : 0.5;
