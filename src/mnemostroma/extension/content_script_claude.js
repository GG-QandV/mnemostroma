...
/**
 * Mnemostroma Content Script for Claude.ai
...
function extractAllMessages() {
  // Claude.ai uses data-testid for messages
...
function notifyBackground() {
  ...
  // Format session_id: claude-ai-<date>
  const today = new Date().toISOString().split('T')[0];
  const sessionId = `claude-ai-${today}`;
...
console.log("Mnemostroma: Content script active on Claude.ai");
