...
| Долговременная память | эпизоды + семантика        | SQLite cold storage + HNSWlib global index  |
...
```
    ...
    HNSWlib Session              HNSWlib Content
...
**Принципиальное различие:** Observer не знает про контент — он видит только смысл процесса. Агент не знает про Observer — он просто делает работу и тегирует её.
...
КОНЕЦ:    async flush → SQLite + HNSWlib update
...
[Шаг 1: Детерминированный фильтр]  ~0.1ms  0MB
  ...
  - anchor signals: имена, даты, координаты
        ...
        │
        ▼
[Шаг 2: GLiNER-small INT8]  ~8ms  42MB
  zero-shot NER для сложных сущностей
  entity_types: ["решение", "запрет", "артефакт", "вопрос", "концепция"]
  (только если Шаг 1 не дал уверенного результата)
        │
        ▼
[Шаг 3: EmbeddingGemma INT8]  ~12ms  52MB
  текст → вектор 512d
        │
        ▼
[Шаг 4: TinyBERT реранкинг]  ~6ms  8MB
...
| `ctx.semantic(query)`   | HNSWlib         | <2ms    | top-K session_ids + scores        |
...
| `content.search(query)` | HNSWlib Content | <2ms    | top-K content_ids                 |
