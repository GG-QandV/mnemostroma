...
[1] CHUNKING — сессионный                                RAM: 0MB
    ...
    │  Параметр: max_chunk_tokens = 256 (оптимум для EmbeddingGemma)
    │  Если output < 256 токенов → чанкинг не нужен (1 чанк)
    │  Если output > 256 → split с overlap 32 токена
    │  Latency: ~0.2ms
    │
    ▼
...
[3] PRECISION LOG — прямая запись                        RAM: ~3MB (данные)
    ...
    │
    ▼
[4] GLiNER-medium-v2.1 ONNX INT8            ★ v1.2      RAM: 95MB (dedicated)
    │  Функция: chunk → entities[] zero-shot NER
    │  Кто: Observer шаг 2 (~30% чанков)
    │  Latency: ~18ms per chunk
...
[5] NER ВЕРИФИКАЦИЯ — двухступенчатая                    RAM: 0MB доп.
    ...
    │  Кто: Observer шаг 2б
    │  Latency: +2ms (переиспользует [7] EmbeddingGemma)
    │
    ▼
[6] КОМПРЕССИЯ ЧАНКА → BRIEF + TAGS                      RAM: 0MB
    │  Функция: chunk (256 токенов) → brief (50 символов) + tags[]
    ...
    │         (экономия: 256 токенов → ~15 токенов)
    │
    ▼
[7] TOKENIZER — Rust tokenizers                          RAM: 15MB (shared)
    │  Функция: brief+tags → input_ids
    │  Кто: Observer шаг 3
    │  Latency: ~0.2ms (короткий текст после компрессии)
    │
    ▼
[8] EmbeddingGemma-300m ONNX INT8                        RAM: 52MB (dedicated)
    │  Функция: input_ids → вектор 512d float16
    │  Кто: Observer шаг 3
    │  Latency: ~12ms
    │  Вход:  brief+tags (~15 токенов) — НЕ raw 256 токенов
    │  Выход: float16[512] — вектор сессии
    │  Переиспользуется в ветках 2, 3, 4 — 0MB доп.
    │
    ▼
[9] PCA / РАЗМЕРНОСТЬ (опционально)                      RAM: 0MB
    │  Функция: 512d → 256d при давлении RAM
    │  Кто: Dissolver при ram_usage > soft_limit
    │  Метод: numpy PCA, offline (не в критическом пути)
    │  Потеря точности: < 1% (MRL свойство EmbeddingGemma)
    │  Latency: N/A (фоновый процесс)
    │
    ▼
[10] HNSWlib Session Index                               RAM: ~10MB (данные)
...
[13] CHUNKING — контентный                               RAM: 0MB
    ...
    │    chapter  → по разделам
    │  max_chunk_tokens = 512 (BGE-M3 оптимум)
...
[14] LZ4 COMPRESS (raw content)                          RAM: 0MB
    ...
    │
    ▼
[15] TOKENIZER — Rust tokenizers                         RAM: 15MB (shared с [7])
    │  Функция: chunk → input_ids (BGE-M3 токенизация)
    │  Latency: ~0.5ms per chunk
    │
    ▼
[16] BGE-M3 dense-only ONNX INT8            ★ v1.2      RAM: 95MB (dedicated)
    │  Функция: input_ids → вектор 512d float16 per chunk
    │  Кто: Agent Content
...
[17] CHUNK POOLING — агрегация чанков                    RAM: 0MB
    ...
    │  Выход: float16[512] — финальный вектор контентного блока
    │
    ▼
[17а] ВЕРИФИКАЦИЯ ТЕГОВ                                  RAM: 0MB доп.
    │  Функция: cosine(embed(tag), vec_block) > 0.65 ?
    │  Кто: Agent Content
    │  Переиспользует [8] EmbeddingGemma для тегов
    │  Latency: +1ms per tag
    │
    ▼
[18] ВЕРСИОННЫЙ DIFF                                     RAM: 0MB
    ...
    │  Latency: <1ms
    │  Примечание: заменяет ColBERT из v1.0
    │
    ▼
[19] HNSWlib Content Index                               RAM: ~2MB (данные)
...
[22] QUERY EXPANSION (опционально)                       RAM: 0MB
    ...
    │
    ▼
[23] TOKENIZER                                           RAM: 15MB (shared)
    │  Latency: ~0.5ms
    │
    ▼
[24] EmbeddingGemma-300m ONNX INT8                       RAM: 52MB (shared с [8])
    │  Функция: query → вектор 512d
    │  Latency: ~12ms
    │
    ▼
[25] HNSWlib Session Index — knn_query                   RAM: ~10MB (shared с [10])
...
[26] ПРЕДФИЛЬТР кандидатов                               RAM: 0MB
    ...
    │
    ▼
[27] TinyBERT-L-2-v2 ONNX INT8                          RAM: 8MB (dedicated)
...
[28] SCORE ФИНАЛЬНЫЙ                                     RAM: 0MB
    ...
    │
    ▼
[29] RAM dict lookup + lazy load                         RAM: shared с [11]
...
[4]   GLiNER-medium INT8    ★+53MB 1,3     Observer      95MB        ~18ms
[5]   NER верификация              1,3     Observer       0MB доп.   ~2ms
[6]   Компрессия → brief+tags      1       Observer       0MB        ~0.3ms
[7]   Tokenizer (shared)           1,2,3   Observer/Agent 15MB       ~0.2ms
[8]   EmbeddingGemma INT8          1,2а,3  Observer/Agent 52MB       ~12ms
[9]   PCA 512→256 (опц.)           1       Dissolver      0MB        фоново
...
[15]  Tokenizer BGE-M3 (shared)    2       Agent           0MB доп.   ~0.5ms
[16]  BGE-M3 dense-only INT8 ★-50MB 2     Agent          95MB        ~12ms
...
[24]  EmbeddingGemma (shared)      3       Agent          52MB shared ~12ms
