...
```
mnemostroma install     → скачивает модели (~342MB)
mnemostroma start       → запускает daemon
mnemostroma status      → дашборд
...
Содержимое .msi:
  ...
  ├── models/                   ONNX INT8 модели (~342MB)
  │   ├── embeddinggemma-300m-int8.onnx
  │   ├── gliner-small-v2.1-int8.onnx
  │   ├── bge-m3-int8.onnx
  │   └── tinybert-l2-v2-int8.onnx
