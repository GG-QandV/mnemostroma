...
# Cyclomatic complexity > 15 (radon if installed)
...
# Sensitive files check (should not be in git)
...
# ══════════════════════════════════════════════════════
header "5. ARCHITECTURE"

# conductor.py must not import mcp/starlette
...
# ipc_server.py only stdlib imports allowed
...
# Adapters must not import conductor directly
...
# config_default.json validity check
if ! "$VENV/python" -c \
