# MNEMOSTROMA SYSTEM ASSESSMENT (v2.3.2)

## Architecture, Port Allocation & Resource Footprint

> **Assessment note**: v2.3.2 embeds SSE and HTTP MCP adapters inside the daemon process — eliminating two separate processes (~65 MB RAM saved). Port conflict protection added. 831 tests passing.
...
| `8765` | `127.0.0.1`    | HTTP/SSE      | Bearer Token  | Cursor, Claude Code, Grok, Perplexity        | SSE MCP transport                        |
...
| `8769` | `127.0.0.1`    | HTTP          | OAuth / Token | Remote: Claude.ai, ChatGPT, Perplexity       | MCP OAuth Adapter + tunnel               |
